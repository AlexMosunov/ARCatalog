//
//  ARFurnitureViewController+Actions.swift
//  ARCatalog
//
...
extension ARFurnitureViewController {
    func placeVirtualObject(_ virtualObject: VirtualObject) {
        let trackedRaycast = createTrackedRaycastAndSet3DPosition(of: virtualObject, from: virtualObject.raycastQuery!,
    ...
    @objc func pinch(sender: UIPinchGestureRecognizer) {
        ...
        if !hitTest.isEmpty {
            ...
            //            guard model.selectedItem?.name == node.name else {return} //TODO: refactor
            guard node.parent?.name != "FocusSquare" && node.name != nil else { return }
            let pinchAction = SCNAction.scale(by: sender.scale, duration: 0)
            print(sender.scale)
            if let parent = node.parent {
                parent.runAction(pinchAction)
            } else {
